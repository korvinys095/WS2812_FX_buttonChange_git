...
void one_color_allHSV(int ahue) {    //-SET ALL LEDS TO ONE COLOR (HSV)
  for (int i = 0 ; i < LED_COUNT; i++ ) {
    //setColor(i, hsv2rgb(ahue, thissat, 255, col));
    //leds[i] = CHSV(ahue, thissat, 255);
    hsv2rgb(ahue, thissat, 255, col);
    leds[i] = CRGBW(col[0], col[1], col[2], 0);
  }
}
...
void fill_Wrainbow( struct CRGBW * pFirstLED, int numToFill,
                  uint8_t initialhue,
                  uint8_t deltahue = 5 )
{
    CHSV hsv;
...
void new_rainbow_loop() {                      //-m88-RAINBOW FADE FROM FAST_SPI2
  ihue -= 1;
  fill_Wrainbow( leds, LED_COUNT, ihue );
  LEDS.show();
  if (safeDelay(thisdelay)) return;
}
/*
void demo_modeA() {
  ...
  one_color_all(255, 255, 255, 0); LEDS.show(); if (safeDelay(1200)) return;
  ...
  one_color_all(0, 0, 0, 0); LEDS.show();
  ...
  one_color_all(255, 0, 0, 0); LEDS.show(); if (safeDelay(1200)) return;
  one_color_all(0, 255, 0, 0); LEDS.show(); if (safeDelay(1200)) return;
  one_color_all(0, 0, 255, 0); LEDS.show(); if (safeDelay(1200)) return;
  one_color_all(255, 255, 0, 0); LEDS.show(); if (safeDelay(1200)) return;
  one_color_all(0, 255, 255, 0); LEDS.show(); if (safeDelay(1200)) return;
  one_color_all(255, 0, 255, 0); LEDS.show(); if (safeDelay(1200)) return;
}
